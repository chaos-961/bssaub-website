...
- **Current phase:** all 5 phases shipped; **v0.0.2 pushed** (2026-07-23, authorized "update and push"). **Every §8 budget now passes in production** — live H2 measurement killed the LCP question: 2.29 s median vs 2.5 s budget (the local 3.3 s was an HTTP/1.1 preview artifact; §15). Remaining site work is content-blocked on the user.
- **Version:** **0.0.2** (`version.json` + all three static badge fallbacks in sync per §1.4).
- **Blockers (all user-owed):** **Story/Mission/Vision text** (§13.1 — photos exist, copy does not; drops straight into the three journey cards) · corrected sponsor list incl. The Kalm Studio question (§15 notes) · Design Lab call (§13.3) · Dunkin' redemption details · Become-a-Sponsor Google Form URL (§6.6 — CTA runs on interim mailto until then). Resolved: headline · card art · journey imagery · palette (§13.4, `#881532` extracted) · **LCP budget (met in production, 2.29 s)**.
