...
- **Current phase:** Phase 5 — built and audited. **v1.0.0-candidate quality, two open calls:** (1) LCP 3.26 s vs the 2.5 s budget — a genuine §6.1-vs-§8 design tension, levers listed in §15; (2) Story/Mission/Vision copy still owed (§13.1). Everything else passes. Awaiting the user's own-device sign-off, content answers, and — separately, per §1.2 — explicit commit/push authorization for the first deploy.
- **Version:** 0.0.1 staged. Ships on first authorized push, bumps +0.0.1 before every push after.
