...
- **Current phase:** all 5 phases shipped; **v0.0.6 pushed** (2026-07-23, push authorized in-message: "…push"). Journey geometry round — **checkpoint dots now ride the media** (measured px beside each image on desktop, ON the image edge alternating sides on mobile, always centered on the frame; path generated from the dots, so **mobile serpentines like desktop** — straight rail retired) · **sponsor popup never scrolls** (compact tiers at ≤784px/≤576px tall + a two-column landscape layout; verified fit at 6 viewport classes) · latent `[hidden]`-vs-`display:grid` modal bug fixed. Standing rules: no hyphens/dashes in visible copy · dropdown nav · feel checks on real hardware are the user's.
- **Version:** **0.0.6** (`version.json` + all three static badge fallbacks in sync per §1.4).
