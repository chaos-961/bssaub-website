...
- **Current phase:** all 5 phases shipped; **v0.0.3 pushed** (2026-07-23, push authorized in-message: "Add these … and push"). §13.1 Story/Mission/Vision text landed (trimmed ~30% per the user) · journey section overhauled (glowing-orb marker, once-only reveals fixing the disappearing-card bug, 5 juice adds) · **all 25 sponsors in the field** (8 badge-less pending discount confirmation) · left category ruler + mobile chip removed (user call; zone headings stay).
- **Version:** **0.0.3** (`version.json` + all three static badge fallbacks in sync per §1.4).
