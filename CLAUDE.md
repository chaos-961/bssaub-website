...
- **Current phase:** all 5 phases shipped; **v0.0.5 pushed** (2026-07-23, push authorized in-message: "push after u finish everything I told u"). Quality round — **sponsor modal redesigned** (ringed logo, oblique discount pill, numbered step chips, full-width CTA, thin scrollbar, null-discount guard) · **hamburger is now a dropdown under the button** (user call — the §6.2 full-screen overlay is retired; ESC/outside/scroll/focusout close, no scroll lock) · **background rebuilt** (aurora wine blobs at 1/8-res upsample + stronger dust/shimmer + static CSS washes + film grain; reduced motion keeps the static depth) · **Our Story compacted again** (2 paragraphs, 62 words) · **standing copy rule: NO hyphens or dashes in any user-visible text** (titles use "·", compounds rephrased; verified by DOM sweep — applies to all future copy).
- **Version:** **0.0.5** (`version.json` + all three static badge fallbacks in sync per §1.4).
