...
- **Current phase:** all 5 phases shipped; **v0.0.8 pushed** (2026-07-23, push authorized in-message: "…push"). Journey path, fourth round: **desktop got chaos** (hashed entry/exit bows + overshoot curls in every inter-frame gap — sweep 95→182 px, 9 direction reversals — still 0 samples over any image or text column) · **mobile checkpoints centered on their images** (they sat above the frames; rail x stays CSS-owned, JS sets measured y — line still perfectly straight). Standing rules: no hyphens/dashes in copy · dropdown nav · phone timeline = straight rail, dots at image centers · feel checks on real hardware are the user's.
- **Version:** **0.0.8** (`version.json` + all three static badge fallbacks in sync per §1.4).
...
- **2026-07-23 · 0.0.8 pushed ·** **Desktop chaos + mobile checkpoint centering** (user: "on desktop the timeline should be more chaotic, it felt too ordered" + screenshot showing mobile dots sitting ABOVE their images — "should be near the image, middle of it, for every section … push"). **Desktop:** the waypoint builder now adds hashed wander — per-node entry/exit bows (12–36 px, always pointing AWAY from that node's image) and an **overshoot curl** in every inter-frame gap ≥90 px (swings 26–62 px past the next checkpoint's x at a hashed fraction of the gap, then comes back), plus a jittered start x and one last sway on the Join approach. All wander x is clamped to the mid corridor (36–64% of track width — between the two text columns), and bows only ever point away from frames, so the v0.0.7 no-overlap guarantee survives: **0 of 600 samples inside any image rect (+6 px) AND 0 inside any text-content rect**; sweep 95→182 px; true x-direction reversals 9 (plateau-aware metric — the first count of 0 was a broken metric skipping zero-delta pairs, not a straight path). Amounts are index-hashed, not random — the shape is stable across rebuilds/resizes, so ignition prefix lengths can't drift mid-scroll; binary-searched first-lit progress still lands ≤2 px from every dot. **Mobile:** dots keep the CSS rail x (18px → x=38 track space, line sweep 0.0 px) but journey.js now sets their y inline to the measured image center — 0 px vertical error on all three content nodes (they previously sat at a fixed top:26px, above every frame — the user's screenshot); the mobile top:26/margin-top:0 CSS died, the Join dot's seat re-expressed as top:20px against the base −8 px margin (same visual position). Zero console errors; build **91.98 KB JS gz of 100**. v0.0.8 synced (version.json + 3 badges).
