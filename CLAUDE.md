...
- **Current phase:** all 5 phases shipped; **v0.0.4 pushed** (2026-07-23, push authorized in-message: "do what u think is good, and push"). Perk Field juice round — grab-and-throw · jelly squash-stretch · impact rim-flashes + contact sparks · stir currents · idle breathing/shivers · hover make-room — plus a new **ambient background** (scroll-reactive dust + sea-shimmer streaks, `ambient.js`) and a pointer-repulsor coordinate-frame bugfix. Feel on real hardware remains the user's check; all knobs sit in `perkField.js` TUNING.
- **Version:** **0.0.4** (`version.json` + all three static badge fallbacks in sync per §1.4).
