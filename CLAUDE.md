...
## Design system (light theme since v0.0.9, aurora background since v0.1.0, literal aurora since v0.1.7)

The site went light on the user's word (2026-07-23); the old dark background system (dark aurora canvas, ground washes, film grain) is deleted for good — do not resurrect that look. The background became a LITERAL aurora at v0.1.7 (user reference: a northern lights image search, same day) after two subtle rounds read as plain white on their hardware — never ship background tint peaks under ~0.14 for this user's display. Two layers in brand colors over the `#fcfaf8` ground, both inside ONE `div.site-bg` first in `<body>` on all three pages (`position: fixed; inset: 0; z-index: -1; pointer-events: none`, base `scale(1.05)` overscan). Layer one, the CSS ribbons (`src/styles/aurora.css`): four rotated feathered radial-gradient band children — dusty rose curtain, maroon undercurrent (`rgb(136 21 50)` sampled from the CTA), low mauve band, high rose wisp — plus champagne core and cream veil on the parent; vmax/% sizing only, every band transparent by 72-76% of its radius (no hard edges), transform-only sway on prime durations (53/67/71/83s, 120s parent drift). This layer is the instant first paint and the complete fallback. Layer two, the WebGL curtain shader (`src/modules/auroraSky.js`, index + account; 404 stays ~1KB JS on ribbons alone): flowing ray-striated curtains and champagne glow in the same palette, pointer parallax, half-resolution render capped for 4K, ~30fps gate, low-power context, highp-with-mediump-fallback precision and bounded time, pauses when the tab hides, removes itself on context loss; it fades in over the ribbons as `.site-bg__canvas`. Color mix caps: rose .36, maroon .16, mauve .26, champagne .30. Reduced motion (OS setting or `?reduced-motion`, stamped as `html.reduced-motion` by the inline head script): ribbons freeze at their designed pose, the shader draws one static frame and stops; `.site-bg` reports `data-aurora` = `flowing`/`static`/`css` for QA. Readability floors, re-based for the bold look (models: scratch `ribbon-contrast.mjs` and `shader-contrast.mjs` — grid sweeps, six viewports, drift jitter, eight time samples): shader worst composite `rgb(224 190 194)` holds `--ink` 10.5:1, `--accent` 5.6:1, `--ink-soft` 4.75:1; CSS-only worst `rgb(227 199 197)` holds 11.3/6.0/5.1. Everything clears WCAG AA (4.5); the old 12/6.5/5.5 floors belonged to the quiet designs.
